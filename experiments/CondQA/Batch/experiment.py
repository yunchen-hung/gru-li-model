...
def run(data_all, model_all, env, paths, exp_name):
    ...
    for run_name, data in data_all.items():
        ...
        recall_probability.fit(np.repeat(np.arange(timestep_each_phase).reshape(1,-1), retrieved_memories.shape[0], axis=0), 
        ...
        for i in range(1, timestep_each_phase+1):
            ...
            if len(chosen_trials) < 1:
                continue
            recall_probability.fit(np.repeat(np.arange(timestep_each_phase).reshape(1,-1), chosen_trials.shape[0], axis=0),
        ...
        for j in range(1, timestep_each_phase+1):
            ...
            # print(c_recalling.shape, memory_sequences.shape, sum_features.shape)
