    ...
    for run_name, data in data_all.items():
        ...
        for i in range(5):
            if has_memory:
                ...
                np.argmax(readouts[i]["similarity"].squeeze(), axis=1)+1, rewards[i][sequence_len:]))
            else:
        ...
        #     retrieved_memories.append(readouts[i]['retrieved_memory'].squeeze()[-timestep_each_phase:])
        #     memory_similarity = readouts[i]['similarity']
