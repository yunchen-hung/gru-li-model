...
def run(data_all, model_all, env, paths, exp_name, **kwargs):
    ...
    for run_name, data in data_all.items():
            ...
            for i in range(actions.shape[0]):
                for j in range(actions.shape[1]):
                    if actions[i][j][0] == action_space_shape - 1:
                        ...
                        converted_actions[i][j][:feature_num] = np.array([actions[i][j][0] // (feature_dim ** k) % feature_dim for k in range(feature_num)])
        ...
        results = semantic_contiguity.fit(actions[:, :, :env.unwrapped.num_features], env.unwrapped.feature_dim)
