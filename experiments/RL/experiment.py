...
def run(data_all, model_all, env, paths, exp_name):
    ...
    for run_name in data_all.keys():
        ...
        if run_name_without_num not in sim_mem.keys():
            ...
            forward_asymmetry[run_name_without_num] = []
    ...
    for run_name, data in data_all.items():
        ...
        result_all = recall_probability.get_results_all_time()
        rec_prob_all[run_name_without_num].append(result_all)
        forward_asymmetry[run_name_without_num].append(result_all[env.memory_num] - result_all[env.memory_num-2])
    ...
    for run_name in run_names_without_num:
        ...
        # forward asymmetry - accuracy plot
        plt.figure(figsize=(4, 3), dpi=180)
        plt.scatter(accuracy[run_name], forward_asymmetry[run_name])
        plt.xlabel("accuracy")
        plt.ylabel("forward asymmetry")
        plt.title("accuracy vs forward asymmetry".format(run_num))
        plt.tight_layout()
        savefig(paths['fig']/"mean"/run_name, "accuracy_forward_asymmetry")
