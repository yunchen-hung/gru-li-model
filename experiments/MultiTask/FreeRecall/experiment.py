...
def run(data_all, model_all, env, paths, exp_name):
    ...
    for run_name, data in data_all.items():
        ...
        if hasattr(model, "step_for_each_timestep"):
            ...
            timestep_each_phase = step_for_each_timestep * env.unwrapped.sequence_len
        else:
        ...
        memory_contexts = np.zeros((all_context_num, sequence_len))     # ground truth of memory for each trial
        for i in range(all_context_num):
            memory_contexts[i] = np.array(data['trial_data'][i]["items"])
        ...
        vocabulary_size = env.unwrapped.vocabulary_num
