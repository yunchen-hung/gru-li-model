...
from tasks.ConditionalFreeRecall import ConditionalFreeRecall
from tasks.wrappers import MetaLearningEnv

class TestConditionalFreeRecall(unittest.TestCase):
    def setUp(self):
        # Initialize the environment
        self.env = ConditionalFreeRecall(num_features=4, feature_dim=2, sequence_len=4)
    ...
    def test_step(self):
        ...
        print("condition_feature", self.env.condition_feature)
        print("condition_value", self.env.condition_value)
        print("matched_items", memory_sequence_int[self.env.matched_item_indexes])
        print("time step: ", 0)
        print("obs: ", obs)
        print("info: ", info)
...
class TestMetaLearningEnv(unittest.TestCase):
    def setUp(self):
        ...
        self.env = MetaLearningEnv(ConditionalFreeRecall(num_features=4, feature_dim=2, sequence_len=4))
...
class TestVectorEnv(unittest.TestCase):
    def setUp(self):
        def load_single_environment():
            return MetaLearningEnv(ConditionalFreeRecall(num_features=4, feature_dim=2, sequence_len=4))
