...
class ValueMemoryCTRNN(BasicModule):
    def __init__(self, planning_module: CTRNN, memory_module: ValueMemory, hidden_dim: int, output_dim: int, em_gate_type='vector', 
    em_gate_act_fn='Sigmoid', use_memory=True, decode_memory=False, encode_memory=False, device: str = 'cpu'):
        ...
        if em_gate_type == "scalar":
            self.fc_em_gate = nn.Linear(hidden_dim, 1)
        elif em_gate_type == "vector":
            self.fc_em_gate = nn.Linear(hidden_dim, hidden_dim)
        else:
    ...
    def compute_memory(self, h):
        if self.em_gate_type == "scalar":
            em_gate = self.em_gate_act_fn(self.fc_em_gate(h))
        ...
        if self.em_gate_type == "vector":
            mem_gate = self.em_gate_act_fn(self.fc_em_gate(h))
