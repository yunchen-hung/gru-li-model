...
class TCMRNN(BasicModule):
    ...
    mem_gate_type="vector", output_type="decision", device: str = 'cpu'):
        super().__init__()
        self.device = device
        ...
        self.fc_critic = nn.Linear(hidden_dim * 2, 1)
    ...
    def forward(self, inp, state):
        if self.encoding:
            ...
            if self.output_type == "decision":
                ...
                output = inp
            value = self.fc_critic((torch.cat((c_in.unsqueeze(0), state), 1)))
            ...
            value = self.fc_critic((torch.cat((f_in_inhibit_recall.unsqueeze(0), state), 1)))
            return output, value, state
